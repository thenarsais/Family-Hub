...
| Database | **Supabase managed Postgres**, prod project. Only work is applying migrations 001–026 + the learning & trivia seeds. |
...
2. Apply migrations **001 → 026** in order (the same one-off `pg` script used
   for dev, pointed at the prod `DATABASE_URL`). Skip `003_seed_demo_users.sql`
   — that seeds demo accounts, not wanted in prod.
3. Run **`npm run seed:learning`**, **`npm run seed:trivia`**, **`npm run
   seed:rewards`**, and **`npm run seed:maintenance`** (backend, pointed at the
   prod `DATABASE_URL`) — the first upserts the 177 Gujarati lessons into
   `learning_lessons`, the second the 111 questions into `trivia_questions`,
   the third a 12-item starter reward library into `reward_library`, the
   fourth a 7-item starter maintenance list into `maintenance_items` — each
   for any family that doesn't have one yet. All four idempotent; safe to
   re-run after a content edit.
