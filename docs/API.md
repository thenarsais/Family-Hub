...
| Calendar | `/api/calendar/*` | x-user-id (OAuth callback is unauthenticated by nature) | Family events CRUD, Google Calendar OAuth connect/callback/disconnect, Google event fetch, event dismissal, Google event create/edit/delete (`POST`/`PATCH`/`DELETE /api/calendar/google/events` — parents only), and `POST /api/calendar/photo-import` (FR-147, parents only — one Gemini vision call over a flyer/invite photo, returns draft events only, never writes anything; no-ops with 503 without `GEMINI_API_KEY`). |
