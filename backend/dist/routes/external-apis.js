...
const mw = __importStar(require("../services/merriam-webster"));
const weather = __importStar(require("../services/openweather"));
const sendgrid = __importStar(require("../services/sendgrid"));
 ...
 * Get word definition
 */
router.get('/dictionary/word/:word', verifyAuth, async (req, res) => {
    try {
 ...
 * Get word of the day
 */
router.get('/dictionary/word-of-day', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.get('/dictionary/search', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.get('/weather/city/:city', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.get('/weather/coords', verifyAuth, async (req, res) => {
    try {
 ...
 * Get 5-day forecast
 */
router.get('/weather/forecast/:city', verifyAuth, async (req, res) => {
    try {
        const city = req.params.city;
        if (!city || city.length === 0) {
            return res.status(400).json({ error: 'City parameter is required' });
 ...
 */
router.post('/weather/activity-suitability', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.post('/email/achievement', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.post('/email/points', verifyAuth, async (req, res) => {
    try {
 ...
 */
router.post('/email/daily-summary', verifyAuth, async (req, res) => {
    try {
        ...
        const result = await sendgrid.sendDailySummaryEmail(email, userName, dailyPoints, badgesEarned, streakDays || 0);
        if (!result.success) {
            return res.status(500).json({
                error: 'Failed to send email',
                message: result.error
 ...
 */
router.post('/email/parent-notification', verifyAuth, async (req, res) => {
    try {
        ...
        const result = await sendgrid.sendParentNotificationEmail(email, parentName, childName, pointsEarned, badgesEarned || 0);
        if (!result.success) {
            return res.status(500).json({
                error: 'Failed to send email',
                message: result.error
