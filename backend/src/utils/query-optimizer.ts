 ...
 * Get slow queries
 */
export function getSlowQueries(limit: number = 20): QueryProfile[] {
...
export function getQueryStats() {
  ...
  const avgExecutionTime =
    queryProfiles.reduce((sum, p) => sum + p.executionTime, 0) / totalQueries;
...
export function getOptimizationRecommendations(): string[] {
  ...
  const slowPercentage = parseFloat(
    stats.slowQueryPercentage.replace('%', '')
  );
  if (slowPercentage > 10) {
    recommendations.push(
      `${slowPercentage}% of queries are slow. Consider adding indexes or optimizing slow queries.`
    );
  }
  ...
  if (avgTime > 50) {
    recommendations.push(
      `Average query execution time is ${avgTime}ms. Consider query optimization or caching.`
    );
  }
  ...
  if (avgRows > 1000) {
    recommendations.push(
      `Average query returns ${avgRows} rows. Consider pagination or filtering.`
    );
  }
 ...
 */
export function clearQueryProfiles() {
 ...
 */
export function optimizeBatchQueries(queries: string[]): string[] {
  ...
  for (const query of queries) {
    ...
    if (
      optimizedQuery.toUpperCase().startsWith('SELECT') &&
 ...
 */
export function detectNPlusOneQueries(): Map<string, number> {
